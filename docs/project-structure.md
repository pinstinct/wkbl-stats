...
| `static-hosting-troubleshooting.md` | GitHub Pages 정적 호스팅 트러블슈팅 |

완료된 계획 문서는 `docs/bak/`에 보관:

| 파일                                    | 내용                                 |
| --------------------------------------- | ------------------------------------ |
| `bak/visualization-plan.md`             | 시각화 + 예측 기능 구현 계획         |
| `bak/refactor-plan.md`                  | P0~P4 리팩토링 계획 (전체 완료)      |
| `bak/advanced-stats-plan.md`            | 고급 지표 Tier 1~3 계획              |
| `bak/advanced-stats-display-plan.md`    | 프론트엔드 고급 지표 표시 개선 계획  |
| `bak/players-teams-leaders-fix-plan.md` | Players/Teams/Leaders 버그 수정 계획 |
| `bak/additional-data-plan.md`           | 추가 데이터 수집 조사/계획           |
| `bak/remaining-features.md`             | 미완 기능 통합 목록                  |
...
- 완료된 계획 문서는 `docs/bak/`으로 이동한다. `docs/` 루트에는 활성 참조 문서만 유지한다.
