...
- 3점 코너 직선 위치를 `x=22/269`로 확장하고 반경을 `140`으로 재조정해 실제 3점 시도 분포와 시각 정합 개선
- 골대 근처 제한구역(restricted area) 아크 방향을 수정해 상단 원호가 겹쳐 보이는 렌더링 이슈 완화
- 모바일에서 `chart-card`가 넘치지 않도록 폭/오버플로/캔버스 높이 반응형 규칙 보강
- 박스스코어 상세 테이블에 컬럼별 정렬(선수, MIN, PTS, REB, AST, STL, BLK, TO, FG, 3P, FT, TS%, PIR, +/-) 추가
