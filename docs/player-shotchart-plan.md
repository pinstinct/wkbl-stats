...
- 선수 슛차트 필터 `select` UI 스타일 보강(테두리/패딩/색상)
- 존 필터는 raw `shot_zone` 대신 코트 좌표 기반으로 존을 재분류해 시각 결과와 일치하도록 수정
- 쿼터 차트 라벨 변환 중복 버그 수정 (`QTNaN` 제거)
- 필터 헤더와 차트 섹션 사이 여백을 소폭 추가해 가독성 개선
